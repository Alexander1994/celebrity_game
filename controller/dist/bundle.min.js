...
var ServerConnection = /** @class */ (function () {
    function ServerConnection(onMessage, onConnection) {
        ...
        // reconnect
        this.disconnectDisplay = document.querySelector("#disconnectDisplay");
        this.reconnectCount = 0;
        this.lastMessageDelay = 0;
        this.connectedToGame = false;
        this.startSocketConnection(onMessage, onConnection);
    }
    ServerConnection.prototype.startSocketConnection = function (onMessage, onConnection) {
        this.serverConnection = new WebSocket(this.connectionName);
        //Messages received from the Signaling Server
        this.serverConnection.onmessage = onMessage;
        var that = this;
        this.serverConnection.onopen = function () {
            console.log("Connected to the signaling server.");
            onConnection();
            that.reconnectCount = 0;
            Helpers_1.hide(that.disconnectDisplay);
        };
        this.serverConnection.onerror = function (err) {
            console.log("Got error on trying to connect to the signaling server:", err);
            Helpers_1.display(that.disconnectDisplay);
        };
        this.serverConnection.onclose = function (err) {
            that.reconnectCount++;
            setTimeout(function () {
                that.serverConnection = new WebSocket(that.connectionName);
                that.startSocketConnection(onMessage, onConnection);
            }, Math.pow(8, that.reconnectCount));
            Helpers_1.display(that.disconnectDisplay);
        };
    };
    ServerConnection.prototype.isClosed = function () {
        var conn = this.serverConnection;
        var state = conn.readyState;
        return conn.CLOSED === state || conn.CLOSING === state;
    };
...
},{"../../../env/env":2,"../../../shared/Helpers":8,"../../../shared/MessageTypes":9}],6:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
// Shared
...
function connectToServer() {
    peerConnection = new ServerConnection_1.ServerConnection(onMessage, function () {
...
document.addEventListener("visibilitychange", function () {
    if (!document.hidden && (peerConnection === undefined || peerConnection.isClosed())) {
        connectToServer();
    }
}, false);
function onMessage(event) {
    var message = JSON.parse(event.data);
    peerConnection.setMsgDelay(message);
...
playerDataBtn.addEventListener("click", function (event) {
    ...
    if (name.length >= maxNameSize) {
        alert("please enter a name smaller than " + maxNameSize + " characters");
        return;
    }
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var isDisplayed = function (el) { return el.style.display !== "none"; };
